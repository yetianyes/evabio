...
$x=mysql_real_escape_string ($_GET['yeshu']);
$ziliao1=mysql_query("SELECT * FROM `shiji` ORDER BY  `shiji`.`id` DESC LIMIT 0 , 1");
$row1 = mysql_fetch_row($ziliao1);
if(preg_match("/^\d*$/",$x))   
{ 
if ($x==0or$x==1or$x==2or$x>$row1[0]*2+2) 
{
$y=1;
$result = mysql_query("select * from `shiji`  WHERE  `id` =$y");
$row =mysql_fetch_row($result);
$ziliao2=mysql_query("SELECT * FROM `shiji` WHERE `id`=0 ");
...
$result = mysql_query("select * from `shiji`  WHERE  `id` =$y");
$row =mysql_fetch_row($result);
$ziliao2=mysql_query("SELECT * FROM `shiji` WHERE `id`<$row[0] ORDER BY  `shiji`.`id` DESC LIMIT 0 , 1");
$num2 = mysql_fetch_row($ziliao2);
}
}
else   {echo('不是数字');exit;}
...
var i=<?php echo $num2[0]?>;
$('#mybook').booklet({
width:'100%',height:'100%',pageNumbers:false,manual:true,<?php if($mobile==true){echo 'arrows:true,manual:false,';} else {echo 'manual:true,';}?>
...
$(".ye ").hide(),
$( ".slider" ).slider({
      value: <?php echo $row[0]*2+2?>,
      min: 1,
      max: <?php echo $row1[0]*2+2?>,
